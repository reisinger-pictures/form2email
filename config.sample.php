...
$domains = [
    ...
    'a.com' => [
        ...
        // Optional per-client abuse brake. When 'rate_limit' is absent, no
        // limiting is performed. 'client_ip_header' must be a header the trusted
        // reverse proxy overwrites; X-Forwarded-For is safe with Caddy (which
        // ignores spoofed incoming values), X-Real-IP is not.
        'rate_limit' => [
            'max' => 5,
            'window' => 300,
...
// Loaded ONLY when APP_ENV=development is set explicitly, so production stacks
// (APP_ENV unset) never accept these origins.
if (getenv('APP_ENV') === 'development') {
    // Local development profiles (e.g. Astro dev servers) reuse 'a.com'.
    $domains['localhost:4321'] = 'a.com';
    $domains['localhost:4322'] = 'a.com';
}
