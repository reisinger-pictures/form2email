 ...
 * CLOSED only when the client was actually allowed and has now reached the
 * configured limit.
 *
 * The window logic itself lives in the pure helpers isRateLimited() and
 * pruneTimestamps() in src/functions.php, which are unit-tested separately.
 *
...
function allowRequest(string $storageDir, string $key, int $max, int $window): bool
{
    ...
    try {
        ...
        // Drop everything outside the window before deciding, so stale entries
        // never keep a client locked out beyond the configured window.
        $timestamps = pruneTimestamps($timestamps, $now, $window);

        $allowed = !isRateLimited($timestamps, $max, $now, $window);
        if ($allowed) {
            $timestamps[] = $now;
        }

        // Persist the pruned (and possibly extended) list either way, so the
        // counter file does not grow without bound while a client is blocked.
        ftruncate($handle, 0);
        rewind($handle);
